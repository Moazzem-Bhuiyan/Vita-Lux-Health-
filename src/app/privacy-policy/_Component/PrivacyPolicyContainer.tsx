...
export default function PrivacyPolicyContainer() {
  const { data: privacyData, isLoading } = useGetSettingsQuery({
    type: 'privacy_policy',
  });

  const content = privacyData?.data?.data || '';
        ...
        >
          <div className="inline-flex items-center gap-3 mb-6">
            <div className="p-3 rounded-full bg-[#c9a96e]/10">
              <Lock className="w-10 h-10 text-[#c9a96e]" />
            </div>
          </div>
        ...
        >
          {isLoading ? (
            ...
            <div className="text-center py-20 text-white/50">
              ...
              <p className="text-xl">Privacy Policy will be available soon.</p>
            </div>
          )}
        </motion.div>
      </div>
